...
namespace cs225{
        ...
        void Image::scale (unsigned w, unsigned h){
            ...
            if (factor > double(h)/height()) factor = double(h)/height();
            scale(factor);
...
void Image::rotate(int degree) {
    ...
    if (degree == 90){
        ...
        this->resize(hi, wi);
        for (unsigned w = 0; w < wi; w++) {
            for (unsigned h = 0; h < hi; h++) {
                *(this->getPixel(hi-1-h, w)) = *(pic->getPixel(w,h));
            }
        }
        delete pic;
        return;
        ...
        for (unsigned w = 0; w < wi; w++) {
            for (unsigned h = 0; h < hi; h++) {
                *(this->getPixel(h, wi-1-w)) = *(pic->getPixel(w,h));
            }
        }
        delete pic;
        return;
        ...
        for (unsigned w = 0; w < wi; w++) {
            for (unsigned h = 0; h < hi; h++) {
                *(this->getPixel(wi-1-w,hi-1-h)) = *(pic->getPixel(w,h));
            }
        }
        delete pic;
        return;
