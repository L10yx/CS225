                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    < mergeQueue.front()->freq.getFrequency()) {
        smallest = new TreeNode(singleQueue.front()->freq.getFrequency());
        singleQueue.pop();
    }
    else {
        smallest = new TreeNode(mergeQueue.front()->freq.getFrequency());
        mergeQueue.pop();
    }
    return smallest;
}
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while (singleQueue.size() + mergeQueue.size() > 1)
    {
        ...
        if (singleQueue.empty()) {
            left = mergeQueue.front();
            mergeQueue.pop();
        }
        else if (mergeQueue.empty()) {
            left = singleQueue.front();
            singleQueue.pop();
        }
        ...
        //right
        if (singleQueue.empty()) {
            right = mergeQueue.front();
            mergeQueue.pop();
        }
        else if (mergeQueue.empty()) {
            right = singleQueue.front();
            singleQueue.pop();
        }
        else if (singleQueue.front()->freq.getFrequency() 
            < mergeQueue.front()->freq.getFrequency()) {
        ...
        TreeNode *p = new TreeNode(left->freq.getFrequency() + right->freq.getFrequency());
        p->left = left;
        p->right = right;
        mergeQueue.push(p);
    }
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
        ...
        c = bfile.getNextBit();
        if (c && current->right) current = current->right;
        else if (!c && current->left) current = current->left;        
        if (!current->left && !current->right) {
            ss << current->freq.getCharacter();
            current = root_;
        }
