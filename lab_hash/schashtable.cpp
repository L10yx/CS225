...
{   
    ...
    size_t idx = hash(key, size);
    typename list<std::pair<K, V>>::iterator it = table[idx].begin();
...
{
    typename list<pair<K, V>>::iterator it;
    size_t tsize = findPrime(2 * size);
    list<pair<K, V>>* temp = new list<pair<K, V>>[tsize];
    for (size_t i = 0; i < size; i++) {
        for (it = table[i].begin(); it != table[i].end(); it++) {
            size_t idx = hash(it->first, tsize);
