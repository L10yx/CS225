...
typename List<T>::ListNode* List<T>::split(ListNode* start, int splitPoint) {
    ...
    if (start->next != nullptr) {
      ListNode *secstart = start->next;
      start->next = nullptr;
      secstart->prev = nullptr;
      return secstart;
    } 
    return nullptr;
}
...
typename List<T>::ListNode* List<T>::mergesort(ListNode* start, int chainLength) {
    ...
    int l2 = chainLength/2;
    int l1 = chainLength - l2;
    ListNode *s2 = mergesort(split(start, l1), l2);
    ListNode *s1 = mergesort(start, l1);
    return merge(s1, s2);
}
