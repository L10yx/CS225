...
class BinaryTree
{
  ...
  public:
    ...
    T kthSmallest(Node *subRoot, int order) const;
  ...
  private:
    ...
    bool isOrdered(Node *subRoot, vector<T> &treeVector) const;

    void printPaths(vector<vector<T> > &paths, Node *subRoot) const;
    
    int sumDistances(Node *subRoot) const;

    int sumSons(Node *subRoot) const;
