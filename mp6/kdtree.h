/**
 * @file kdtree.h
 * KDTree implementation using Points in k-dimal space.
 *
 * @author Zesheng Wang
...
/**
 * KDTree class: implemented using Points in Dim dimal space (given
...
class KDTree
{
  private:
     ...
     * Contains left, right children pointers and a K-dimal point
     */
    struct KDTreeNode
    {
      Point<Dim> point;
  ...
  public:
    /**
     * Determines if Point a is smaller than Point b in a given dim d.
     ...
     * @param curDim dim these points are being compared in.
     * @return A boolean value indicating whether the first Point is smaller
     *  than the second Point in the curDim dim.
     */
     ...
     *
     * We are using Euclidean distance to compare k-dimal
    ...
    /**
     * Constructs a KDTree from a vector of Points, each having dim Dim.
     *
     ...
     *     are ties (two points have equal value along a dim), they
     ...
     * the median value in the current dim is selected and a node is
     ...
     * The children pick the median in the next dim, and repeat
     * until the entire set of inputs has been processed. Successive
     * levels of the tree split on increasing dims, modulo the
     * total number: a 3D tree will have levels split by dim 0, 1, 2,
     * 0, 1, 2, etc.
     *
  ...
  private:
     ...
     */
    void _copy(KDTreeNode* subroot, KDTreeNode* otherroot); // copy Treenode
    void _clear(KDTreeNode* subroot); // clear the branch
    KDTreeNode* KDTreeHelper(vector<Point<Dim>>& Points, unsigned left, unsigned right, int dim);
    Point<Dim>& select(vector<Point<Dim>>& list, unsigned left, unsigned right, unsigned k, int dim);
    Point<Dim> findNearestNeighbor(KDTreeNode* subroot, const Point<Dim>& query, int dim) const;
    double distancesquare(const Point<Dim>& target, const Point<Dim>& point) const;
