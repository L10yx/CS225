...
MosaicCanvas* mapTiles(SourceImage const& theSource, vector<TileImage*>& theTiles)
{
    ...
    for (int i = 0; i < (int)theTiles.size(); i++) {
        ...
        map.insert(pair<Point<3>, TileImage*>(point, theTiles[i])); // add to map
    }
    KDTree<3> tree(points); // build tree
    for (int row = 0; row < canvas->getRows(); row++) {
        for (int col = 0; col < canvas->getColumns(); col++) {
            canvas->setTile(row, col, get_match_at_idx(tree, map, theSource, row, col)); // use helperfunction
        }
    }
    return canvas;
}

TileImage* get_match_at_idx(const KDTree<3>& tree, map<Point<3>, TileImage*> tile_avg_map, 
const SourceImage& theSource, int row, int col)
{
    ...
    map< Point<3>, TileImage*>::iterator it = tile_avg_map.find(nearestPoint);
    if (it == tile_avg_map.end())
        cerr << "Didn't find " << avgPoint << " / " << nearestPoint << endl;

    return tile_avg_map[nearestPoint];
}
